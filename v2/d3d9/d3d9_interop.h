...
namespace dxvk {
  ...
  // Restored for the v2 frontend: D3D9InterfaceEx holds this concrete impl of
  // ID3D9VkExtInterface as m_d3d9ExtInterface (newer than the vendor snapshot,
  // which lacks it). The only method, UnlockAdditionalFormats, flips a flag on
  // the parent interface. Lifetime is owned by the parent (AddRef/Release
  // forward to it), matching the sibling interop objects.
