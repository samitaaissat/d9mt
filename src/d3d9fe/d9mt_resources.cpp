...
namespace dxvk {
  ...
  Rc<DxvkResourceAllocation> DxvkMemoryAllocator::createImageResource(
    ...
    // bit); shaderWrite only for genuine storage images.
    //
    // pixelFormatView used to be set on every plain colour texture, to cover
    // the component swizzles sampled views routinely carry (e.g. X8R8G8B8).
    // That was never what the flag is for — the swizzle rides its own
    // newTextureView argument — and it cost lossless compression on every
    // colour texture and render target in the frame (WWDC21 session 10148).
    // It is now set only where a real reinterpretation can happen; see
    // d9mtNeedsPixelFormatView.
    ...
    if (!isDepthStencil && !isCompressed && samples <= 1u
     && d9mtNeedsPixelFormatView(createInfo, caps->wmtFormat))
  ...
  Rc<DxvkResourceAllocation> DxvkImage::allocateStorageWithUsage(
    ...
    // Chain the view-format list: MUTABLE_FORMAT alone says a differing-format
    // view is possible, not which formats, and the allocator needs the formats
    // to decide whether the texture must give up lossless compression
    // (d9mtNeedsPixelFormatView; WWDC21 session 10148). External memory chains
    // are still unnecessary — Metal textures carry no such handles here.
